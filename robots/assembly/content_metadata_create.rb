  ...
  module DorRepo
    module Assembly
      class ContentMetadataCreate < Robots::DorRepo::Assembly::Base
        ...
        def perform(druid)
          obj =  item(druid)
          return LyberCore::Robot::ReturnState.new(status: :skipped, note: 'object is not an item') unless obj.is_item? # not an item, skip

          if obj.stub_content_metadata_exists? && obj.content_metadata_exists? # both stub and regular content metadata exist -- this is an ambiguous situation and generates an error
            raise "#{Dor::Config.assembly.stub_cm_file_name} and #{Dor::Config.assembly.cm_file_name} both exist"
          end

          if obj.content_metadata_exists? # regular content metadata exists -- do not recreate it
