  ...
  module Assembly
    module ContentMetadata
      ...
      def load_content_metadata
        ...
        unless content_metadata_exists?
          raise "Content metadata file #{Dor::Config.assembly.cm_file_name} not found " \
                "for #{druid.id} in any of the root directories: #{@root_dir.join(',')}"
        end
      ...
      def load_stub_content_metadata
        ...
        unless stub_content_metadata_exists?
          raise "Stub content metadata file #{Dor::Config.assembly.stub_cm_file_name} " \
                "not found for #{druid.id} in any of the root directories: #{@root_dir.join(',')}"
        end
